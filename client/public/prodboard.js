...
/**
 * @typedef Options
 * @prop {string} company - company code
 * @prop {string} instance - instance code
 * @prop {string?} host - host page url
 * @prop {string?} environment - constructor url
 */

/**
 * @typedef Instance
 * @prop {function} signIn - sign in
 * @prop {function} signOut - sign out
 * @prop {function} load - load project
 * @prop {function} onInitCompleted - handle planner init completed event
 * @prop {function} send - send command
 * @prop {function} handle - handle event
 */

/**
 * @param element - DOM element where to create constructor instance
 * @param {Options} options
 * @returns {Instance} prodboard instance
 */


window.prodboard = function (element, options) {
    ...
    const getState = function () {
        if (options.start) {
            if (options.start.clone) return options.start.guid + "-clone";
            if (options.start.version) return options.start.guid + ":" + options.start.version;
            return options.start.guid;
        }
        if (mode === "hash")
    ...
    frame.setAttribute("allow", "clipboard-read; clipboard-write; xr-spatial-tracking; camera; accelerometer; gyroscope; magnetometer; fullscreen; web-share");
    ...
    const processMessage = function (event) {
        ...
        if (event.data.command === 'redirect')
            ...
            window.open(event.data.payload);
    ...
    return {
        ...
        load: function (guid, clone, version) {
            const win = frame.contentWindow;
            console.log(guid)
            win.postMessage(
                { command: 'load', payload: { guid: guid, clone: clone, version: version } },
                options.environment);
        },
