...
// логика округления
const normalizeValue = (value: number, allowed: number[]): number => {
    for (let i = 0; i < allowed.length; i++) {
        const current = allowed[i];
        const next = allowed[i + 1];

        const delta = next ? next - current : Infinity;
        const threshold = delta > 1 ? 1 : 0.95;
...
export const getTablePrice = (width: number, height: number, depth: number, priceData: pricePart[]): number => {
    ...
    const hasHeight = priceData.some(el => el.height !== undefined);
    const hasDepth = priceData.some(el => el.depth !== undefined);

    const widths = getAllowedValues(priceData, 'width');
    const heights = hasHeight ? getAllowedValues(priceData, 'height') : [];
    const depths = hasDepth ? getAllowedValues(priceData, 'depth') : [];

    const normWidth = normalizeValue(width, widths);

    const normHeight =
        hasHeight && height !== 0
            ? normalizeValue(height, heights)
            : undefined;

    const normDepth =
        hasDepth && depth !== 0
            ? normalizeValue(depth, depths)
            : undefined;

    const found = priceData.find(el => {
        if (el.width !== normWidth) return false;

        if (hasHeight && height !== 0 && el.height !== normHeight) return false;
        if (hasDepth && depth !== 0 && el.depth !== normDepth) return false;
...
export function getHeightRange(priceData: MaybeUndefined<pricePart[]>, category: productCategory, customHeight: MaybeUndefined<number>): number[] {
    ...
    if (priceData && priceData.find((el) => el.height)) {
        return getAllowedValues(priceData, 'height')
    }
...
function getDepthRange(priceData: pricePart[] | undefined, category: productCategory, customDepth: number | undefined): number[] {
    ...
    if (priceData && priceData.find((el) => el.depth)) {
        return getAllowedValues(priceData, 'depth')
    }
    switch (category) {
        case "Base Cabinets":
        case "Tall Cabinets":
...
const getDoorPriceMultiplier = (materials: RoomMaterialsFormType, is_standard_room: boolean): number => {
    const {door_type, door_finish_material, door_color, category} = materials
...
export const getProductPriceRange = (id: number, materialData: materialDataType): MaybeUndefined<pricePart[]> => {
    ...
    return data.find(i => i.type === base_price_type)?.data;
}
    ...
    switch (id) {
        case 900: {
            switch (custom?.material) {
                case "Milino":
                    price = (width * height * depth / 100) + 120;
                    break;
                case "Plywood":
                ...
                case "Luxe":
                case "Ultrapan PET":
                    price = (width * height * depth / 20) + 120;
                    break;
                case "Ultrapan Acrylic":
        ...
        case 903: {
            const panel = getPanelPrice(area, custom?.material);
            const finishColorCoef = getFinishColorCoefCustomPart(id, custom?.material, color);
            const finishedColorPrice = panel * finishColorCoef;
            const additions = getCustomPriceAdditions(product, values, finishedColorPrice);
            price = finishedColorPrice + additions;
            break;
        }
        case 904: {
            const panel = getPanelPrice(area, custom?.material);
            const finishColorCoef = getFinishColorCoefCustomPart(id, custom?.material, color);
            price = panel * finishColorCoef;
            break;
        }
        case 905: {
            switch (custom?.material) {
        ...
        case 910: {
            price = getShakerPanelPrice(area, custom?.material);
            break
        }
        case 911: {
        ...
        case 912: {
            price = getSlattedPanelPrice(area, custom?.material);
            break
        }
        case 913: {
        ...
        case 918: {
            if (custom?.accessories?.door) price = addToCartDoorAccessories(custom.accessories.door);
            break;
        }
        case 919:
        case 920: {
            if (custom?.standard_doors) price = getCustomPartStandardDoorPrice(custom.standard_doors, type, door_color);
            break
        }
        case 921: {
            if (custom?.standard_panels) {
                ...
                price = getStandardPanelsPrice(custom?.standard_panels, is_price_type_default, apiPanelData)
            }
            break;
        }
        case 922: {
        ...
        case 923: {
            if (custom?.rta_closet) price = getRTAClosetCustomPartPrice(custom.rta_closet, materials);
            break;
        }
        case 924:
        case 925: {
...
const checkProductSize = (customWidth: number, customHeight: number, customDepth: number, sizeLimit: sizeLimitsType, tablePrice: number): boolean => {
...
export const getDrawerInsertsCustomPartPrice = (inserts: MaybeUndefined<DrawerInserts>, width: number): number => {
    if (!inserts || !width) return 0;
    const {box_type, color} = inserts;
    if (!color) return 0;
